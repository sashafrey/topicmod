...
TEST(InstanceManager, Basic) {
  int id = InstanceManager::singleton().Create(artm::InstanceConfig());
  EXPECT_EQ(InstanceManager::singleton().Get(id)->id(), id);

  int id2 = InstanceManager::singleton().Create(artm::InstanceConfig());
  EXPECT_EQ(id2, id+1);
  EXPECT_EQ(InstanceManager::singleton().Get(id2)->id(), id2);

  EXPECT_FALSE(InstanceManager::singleton().TryCreate(id2, artm::InstanceConfig()));
